...
ImageView::~ImageView() { mShader.free(); }
                                     ...
                                     const Vector2f& imageCoordinate)
{
    ...
    // position equation.
    // Round the floating point values to balance out the floating point to
    // integer conversions.
...
{
    if (button == GLFW_MOUSE_BUTTON_MIDDLE && !mFixedOffset) {
...
void ImageView::drawImageBorder(NVGcontext* ctx) const
{
    ...
    nvgStrokeColor(ctx, Color(0.0f, 0.0f, 0.0f, 0.3f));
    nvgStroke(ctx);
    nvgResetScissor(ctx);
    nvgRestore(ctx);
}
