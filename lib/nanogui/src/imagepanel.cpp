...
ImagePanel::ImagePanel(Widget *parent)
    : Widget(parent), mThumbSize(64), mSpacing(10), mMargin(10), mMouseIndex(-1)
{
}
...
void ImagePanel::draw(NVGcontext *ctx)
{
    ...
    for (size_t i = 0; i < mImages.size(); ++i) {
        ...
        NVGpaint imgPaint = nvgImagePattern(ctx, p.x() + ix, p.y() + iy, iw, ih,
                                            0, mImages[i].first,
        ...
        NVGpaint shadowPaint = nvgBoxGradient(
            ctx, p.x() - 1, p.y(), mThumbSize + 2, mThumbSize + 2, 5, 3,
        ...
        nvgBeginPath(ctx);
        nvgRoundedRect(ctx, p.x() + 0.5f, p.y() + 0.5f, mThumbSize - 1,
                       mThumbSize - 1, 4 - 0.5f);

        if (mSelected[i]) {
            nvgStrokeWidth(ctx, 4.0f);
            nvgStrokeColor(ctx, nvgRGBA(0, 255, 0, 80));
        }
        else {
