...
BoxLayout::BoxLayout(Orientation orientation, Alignment alignment, int margin,
                     int spacing)
...
void BoxLayout::performLayout(NVGcontext *ctx, Widget *widget) const
{
    ...
    auto found = children[0]->id().find("ControlsWindow");
    ...
    for (auto w : children) {
        /*        if (w->id().size() > 0) {*/
        // std::cout << "w id " << w->id() << " " << widget->children().size()
        //<< std::endl;
        /*}*/
        /*    auto wChildren = widget->children();*/
        // for (int i = wChildren.size() - 1; i >= 0; i--) {
        /*auto w = wChildren[i];*/
        if (!w->visible()) continue;
        if (first)
            first = false;
        ...
        switch (mAlignment) {
            ...
            case Alignment::Maximum:
                pos[axis2] +=
                    containerSize[axis2] - targetSize[axis2] - mMargin * 2;
                break;
            case Alignment::Fill:
                pos[axis2] += mMargin;
...
Vector2i GroupLayout::preferredSize(NVGcontext *ctx, const Widget *widget) const
{
    ...
    for (auto c : widget->children()) {
        ...
        if (!first) height += (label == nullptr) ? mSpacing : mGroupSpacing;
        first = false;
        ...
        height += targetSize.y();
        width = std::max(width, targetSize.x() + 2 * mMargin +
                                    (indentCur ? mGroupIndent : 0));

        if (label) indent = !label->caption().empty();
    }
    height += mMargin;
...
void GroupLayout::performLayout(NVGcontext *ctx, Widget *widget) const
{
    int height = mMargin,
        availableWidth =
            (widget->fixedWidth() ? widget->fixedWidth() : widget->width()) -
            2 * mMargin;
    ...
    for (auto c : widget->children()) {
        ...
        c->setPosition(
            Vector2i(mMargin + (indentCur ? mGroupIndent : 0), height));
...
void GridLayout::computeLayout(NVGcontext *ctx, const Widget *widget,
                               std::vector<int> *grid) const
{
    ...
    for (auto w : widget->children()) visibleChildren += w->visible() ? 1 : 0;
...
void GridLayout::performLayout(NVGcontext *ctx, Widget *widget) const
{
    ...
    for (int i = 0; i < 2; i++) {
        ...
        for (int s : grid[i]) {
            gridSize += s;
            if (i + 1 < dim[i]) gridSize += mSpacing[i];
        }
        ...
        if (gridSize < containerSize[i]) {
            ...
            for (int j = 0; j < dim[i]; ++j) grid[i][j] += g;
        ...
        for (int i1 = 0; i1 < dim[axis1]; i1++) {
            ...
            for (int j = 0; j < 2; j++) {
                ...
                switch (align) {
                    ...
                    case Alignment::Middle:
                        itemPos[axis] +=
                            (grid[axis][item] - targetSize[axis]) / 2;
                        break;
                    case Alignment::Maximum:
                    ...
                    case Alignment::Fill:
                        targetSize[axis] =
                            fs[axis] ? fs[axis] : grid[axis][item];
                        break;
                }
            }
...
Vector2i AdvancedGridLayout::preferredSize(NVGcontext *ctx,
...
void AdvancedGridLayout::performLayout(NVGcontext *ctx, Widget *widget) const
{
    ...
    if (window && !window->title().empty())
        grid[1].insert(grid[1].begin(),
                       widget->theme()->mWindowHeaderHeight + mMargin / 2);
    else
    ...
    for (int axis = 0; axis < 2; ++axis) {
        ...
        for (Widget *w : widget->children()) {
            ...
            int cellSize =
                grid[axis][anchor.pos[axis] + anchor.size[axis]] - itemPos;
                                       ...
                                       std::vector<int> *_grid) const
{
    ...
    for (int axis = 0; axis < 2; ++axis) {
        ...
        const std::vector<float> &stretch =
            axis == 0 ? mColStretch : mRowStretch;
        grid = sizes;
        ...
        for (int phase = 0; phase < 2; ++phase) {
            for (auto pair : mAnchor) {
                ...
                if ((anchor.size[axis] == 1) != (phase == 0)) continue;
                int ps = w->preferredSize(ctx)[axis], fs = w->fixedSize()[axis];
                int targetSize = fs ? fs : ps;
        ...
        float totalStretch =
            std::accumulate(stretch.begin(), stretch.end(), 0.0f);
        if (currentSize >= containerSize[axis] || totalStretch == 0) continue;
