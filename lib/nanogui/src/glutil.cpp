...
{
    if (shader_string.empty()) return (GLuint)0;

    if (!defines.empty()) {
        if (shader_string.length() > 8 &&
            shader_string.substr(0, 8) == "#version") {
            ...
            oss << line << std::endl;
            oss << defines;
            while (std::getline(iss, line)) oss << line << std::endl;
                             ...
                             const std::string &geometry_fname)
{
    ...
    return init(name, file_to_string(vertex_fname),
                file_to_string(fragment_fname), file_to_string(geometry_fname));
}
                    ...
                    const std::string &geometry_str)
{
    ...
    for (auto def : mDefinitions)
        defines += std::string("#define ") + def.first + std::string(" ") +
                   def.second + "\n";
    ...
    if (!geometry_str.empty() && !mGeometryShader) return false;
    ...
    if (mGeometryShader) glAttachShader(mProgramShader, mGeometryShader);
...
GLint GLShader::attrib(const std::string &name, bool warn) const
{
    ...
    if (id == -1 && warn)
        std::cerr << mName << ": warning: did not find attrib " << name
                  << std::endl;
    return id;
}

void GLShader::setUniform(const std::string &name, const GLUniformBuffer &buf,
                          bool warn)
{

    std::cout << "SetUniform " << std::endl;
    GLuint blockIndex = glGetUniformBlockIndex(mProgramShader, name.c_str());
    if (blockIndex == GL_INVALID_INDEX) {
        if (warn)
            std::cerr << mName << ": warning: did not find uniform buffer "
                      << name << std::endl;
        return;
    }
...
GLint GLShader::uniform(const std::string &name, bool warn) const
{
    ...
    if (id == -1 && warn)
        std::cerr << mName << ": warning: did not find uniform " << name
                  << std::endl;
    return id;
}
...
void GLShader::downloadAttrib(const std::string &name, size_t size,
                              int /* dim */, uint32_t compSize,
                           ...
                           const std::string &_as)
{
    ...
    if (it == otherShader.mBufferObjects.end())
        throw std::runtime_error("shareAttribute(" + otherShader.mName + ", " +
...
{
    for (auto &buffer : mBufferObjects) buffer.second.version = -1;
}
...
void GLShader::free()
{
    for (auto &buf : mBufferObjects) glDeleteBuffers(1, &buf.second.id);
...
void GLUniformBuffer::init() { glGenBuffers(1, &mID); }
...
void GLFramebuffer::init(const Vector2i &size, int nSamples)
{
    ...
    if (nSamples <= 1)
        ...
        glRenderbufferStorageMultisample(GL_RENDERBUFFER, nSamples, GL_RGBA8,
                                         size.x(), size.y());
    ...
    if (nSamples <= 1)
        ...
        glRenderbufferStorageMultisample(
            GL_RENDERBUFFER, nSamples, GL_DEPTH24_STENCIL8, size.x(), size.y());

    glGenFramebuffers(1, &mFramebuffer);
    glBindFramebuffer(GL_FRAMEBUFFER, mFramebuffer);

    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
                              GL_RENDERBUFFER, mColor);
...
{
    glBindFramebuffer(GL_FRAMEBUFFER, mFramebuffer);
    if (mSamples > 1) glEnable(GL_MULTISAMPLE);
}

void GLFramebuffer::release()
{
    if (mSamples > 1) glDisable(GL_MULTISAMPLE);
...
void GLFramebuffer::downloadTGA(const std::string &filename)
{
    ...
    std::cout << "Writing \"" << filename << "\" (" << mSize.x() << "x"
              << mSize.y() << ") .. ";
    std::cout.flush();
    ...
    if (tga == nullptr)
        throw std::runtime_error(
            "GLFramebuffer::downloadTGA(): Could not open output file");
    fputc(0, tga); /* ID */
