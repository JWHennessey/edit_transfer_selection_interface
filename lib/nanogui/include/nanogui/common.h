...
/* Force usage of discrete GPU on laptops (macro must be invoked in main
 * application) */
#if defined(_WIN32)
...
#endif
#pragma warning(disable : 4251)  // warning C4251: class X needs to have
                                 // dll-interface to be used by clients of class
                                 // Y
#pragma warning(disable : 4714)  // warning C4714: function X marked as
...
#ifdef __APPLE__
/// If on OSX, maps to ``GLFW_MOD_SUPER``.  Otherwise, maps to
/// ``GLFW_MOD_CONTROL``.
...
enum class Cursor {
    ...
    HResize,
    VResize,
    /// Not a cursor --- should always be last: enables a loop over the cursor
    /// types.
    CursorCount
};
 ...
 * convenient
 * methods and terminology for thinking of it as a color.  The data operates in
 * the
...
class Color : public Eigen::Vector4f {
   ...
   public:
     ...
     *
     * \param color
     * The three dimensional integer vector being copied, will be divided by
     * ``255.0``.
     *
     * \param alpha
     * The value to set this object's alpha component to, will be divided by
     * ``255.0``.
     */
    ...
    /// Construct a color vector from MatrixBase (needed to play nice with
    /// Eigen)
    template <typename Derived>
    Color(const Eigen::MatrixBase<Derived> &p) : Base(p)
    {
    }
    ...
    {
        float luminance =
            cwiseProduct(Color(0.299f, 0.587f, 0.144f, 0.f)).sum();
 ...
 */
extern NANOGUI_EXPORT std::string file_dialog(
...
extern NANOGUI_EXPORT int __nanogui_get_image(NVGcontext *ctx,
