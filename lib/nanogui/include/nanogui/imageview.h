...
class NANOGUI_EXPORT ImageView : public Widget {
   ...
   public:
    ...
    /// Calculates the image coordinates of the given pixel position on the
    /// widget.
    ...
    /// Calculates the position inside the widget for the given image
    /// coordinate.
    Vector2f positionForCoordinate(const Vector2f& imageCoordinate) const;

    /**
     * Modifies the internal state of the image viewer widget so that the pixel
     * at the provided
     * position on the widget has the specified image coordinate. Also clamps
     ...
     * sensitivity member variable.
     * The scaling occurs such that the image coordinate under the focused
     * position remains in
