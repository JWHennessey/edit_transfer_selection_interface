...
struct type_traits<half_float::half> {
    enum { type = GL_HALF_FLOAT, integral = 0 };
};
template <typename T>
...
// this friendship breaks the documentation
   ...
   public:
    ...
    /// Upload an Eigen matrix as a vertex buffer object (refreshing it as
    /// needed)
    template <typename Matrix>
    void uploadAttrib(const std::string &name, const Matrix &M,
                      int version = -1)
    {
        uint32_t compSize = sizeof(typename Matrix::Scalar);
        GLuint glType =
            (GLuint)detail::type_traits<typename Matrix::Scalar>::type;
        bool integral =
            (bool)detail::type_traits<typename Matrix::Scalar>::integral;

        uploadAttrib(name, (uint32_t)M.size(), (int)M.rows(), compSize, glType,
                     integral, M.data(), version);
    }
    ...
    {
        auto it = mBufferObjects.find(name);
        if (it == mBufferObjects.end()) return false;
        return true;
    }

    /// Create a symbolic link to an attribute of another GLShader. This avoids
    ...
    {
        auto it = mBufferObjects.find(name);
        if (it == mBufferObjects.end()) return -1;
    ...
    {
        auto it = mBufferObjects.find(name);
        if (it != mBufferObjects.end()) it->second.version = -1;
    }
    ...
    template <typename T>
    void setUniform(const std::string &name, const Eigen::Matrix<T, 4, 4> &mat,
                    bool warn = true)
    {
        glUniformMatrix4fv(uniform(name, warn), 1, GL_FALSE,
                           mat.template cast<float>().data());
    }

    /// Initialize a uniform parameter with an integer value
    template <typename T,
              typename std::enable_if<detail::type_traits<T>::integral == 1,
                                      int>::type = 0>
    ...
    template <typename T,
              typename std::enable_if<detail::type_traits<T>::integral == 0,
                                      int>::type = 0>
    void setUniform(const std::string &name, T value, bool warn = true)
    {
    ...
    template <typename T,
              typename std::enable_if<detail::type_traits<T>::integral == 1,
                                      int>::type = 0>
    void setUniform(const std::string &name, const Eigen::Matrix<T, 2, 1> &v,
                    bool warn = true)
    {
    ...
    template <typename T,
              typename std::enable_if<detail::type_traits<T>::integral == 1,
                                      int>::type = 0>
    void setUniform(const std::string &name, const Eigen::Matrix<T, 3, 1> &v,
                    bool warn = true)
    {
    ...
    void setUniform(const std::string &name, const Eigen::Matrix<T, 3, 1> &v,
                    bool warn = true)
    {
        glUniform3f(uniform(name, warn), (float)v.x(), (float)v.y(),
                    (float)v.z());
    }
    ...
    template <typename T,
              typename std::enable_if<detail::type_traits<T>::integral == 1,
                                      int>::type = 0>
    void setUniform(const std::string &name, const Eigen::Matrix<T, 4, 1> &v,
                    bool warn = true)
    {
        glUniform4i(uniform(name, warn), (int)v.x(), (int)v.y(), (int)v.z(),
                    (int)v.w());
    }
    ...
    void setUniform(const std::string &name, const Eigen::Matrix<T, 4, 1> &v,
                    bool warn = true)
    {
        glUniform4f(uniform(name, warn), (float)v.x(), (float)v.y(),
                    (float)v.z(), (float)v.w());
    }

    /// Initialize a uniform buffer with a uniform buffer object
    void setUniform(const std::string &name, const GLUniformBuffer &buf,
                    bool warn = true);
    ...
    {
        size_t size = 0;
        for (auto const &buf : mBufferObjects) size += buf.second.size;
        return size;
    }
...
class UniformBufferStd140 : public std::vector<uint8_t> {
   public:
    ...
    void push_back(T value)
    {
        ...
        for (int i = 0; i < sizeof(T); i++) Parent::push_back(tmp[i]);
    }
    ...
    void push_back(const Eigen::MatrixBase<Derived> &value)
    {
        ...
        int i;
        for (i = 0; i < n; ++i) push_back(value[i]);
        const int pad = n == 1 ? 1 : (n == 2 ? 2 : 4);
        while ((i++) % pad != 0) push_back((typename Derived::Scalar)0);
    }
                   ...
                   bool colMajor = true)
    {
        ...
        for (int i = 0; i < m; ++i) {
            ...
            while ((j++) % pad != 0) push_back((typename Derived::Scalar)0);
        }
    }
};
...
class NANOGUI_EXPORT GLFramebuffer {
   public:
    ...
    /// Create a new framebuffer with the specified size and number of MSAA
    /// samples
    ...
    /// Quick and dirty method to write a TGA (32bpp RGBA) file of the
    /// framebuffer contents for debugging
    void downloadTGA(const std::string &filename);

    GLuint getId() { return mFramebuffer; }
...
struct Arcball {
    ...
    bool motion(Vector2i pos)
    {
        ...
        if (v0.squaredNorm() > 1e-4f && v1.squaredNorm() > 1e-4f) {
            ...
            if (!std::isfinite(mIncr.norm())) mIncr = Quaternionf::Identity();
        }
        return true;
    }
 ...
 *     The far plane.
 */
extern NANOGUI_EXPORT Matrix4f ortho(float left, float right, float bottom,
 ...
 *     The far plane.
 */
extern NANOGUI_EXPORT Matrix4f frustum(float left, float right, float bottom,
                                       float top, float nearVal, float farVal);
/**
